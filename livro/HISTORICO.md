...
- **A intuição acertou o ponto de arquitetura**: o livro **já tinha** como provar identidade desde a spec 080 — o link mágico prova posse da caixa de e-mail. Inventar um segundo mecanismo, pior que o primeiro, era o que estava em vigor.
- **Dois fatores, e cada um cobre a fraqueza do outro.** `ADMIN_EMAILS` diz quem pode ser editor (posse da caixa, provada pelo link); `ADMIN_SENHA` re-prova **intenção no momento**, porque `session_id` é credencial ao portador e um `cmp_sid` copiado de um navegador esquecido não pode exportar a lista de contato. Eu havia proposto outra coleira — exigir entrada recente, com coluna nova em `readers`; a senha, sugerida pelo editor, cobre o mesmo risco **sem mexer no schema**.
- **O silêncio é parte do desenho**: e-mail fora da lista, senha errada e área desligada devolvem a **mesma** resposta, e as duas checagens sempre rodam antes do `and` — sair mais cedo transformaria o tempo de resposta em oráculo. O `/editor` também **não entra na paleta de comandos**: quem não é editor não precisa saber que a área existe.
- **A primeira coisa que a área mostra são as sugestões dos leitores**, e por um motivo: a edição 0.75 registrou que elas *"nunca chegaram ao autor"* pelo bloqueio de SMTP e que *"a sugestão sempre esteve salva no banco"*. Esta pode ser a primeira vez que alguém as lê.
- **`/health` ganha `admin`** em estado (`token`, `emails`, `senha`), nunca valor. Sem isso não dava para distinguir "rota desligada" de "rota protegida" — as duas respondem 403 —, e o editor não tinha como conferir se a variável chegou ao serviço certo. É o remédio das specs 085/086 na última variável que não o tinha.
- **Verificação**: 23 testes de backend novos (141 no total: 118 + 23) e 12 asserções em navegador. Os testes que mais importam são os negativos — sem variável nada existe, e a recusa é indistinguível entre as três causas.
- **Três vezes seguidas, hoje, uma asserção minha estava errada e a página certa**: a última foi `innerText` devolver os títulos em **maiúsculas**, porque devolve o texto *renderizado* e o CSS aplica `text-transform`. Antes disso, o seletor pegou o textarea oculto de sugestão, e o meu próprio rate-limit por IP me barrou depois de cinco tentativas — o limite funcionando. Vale como método: quando o teste reprova, a primeira hipótese é o teste.
- **IA (A3)**: agente **Claude Code (Anthropic)**.

### Edição 0.79 — 2026-08-10 · um endereço só, e uma afirmação nossa que não era verdade (spec 095)
- **Correção da edição 0.76.** Lá está escrito que `SITE_URL` "passa a controlar canonical, hreflang, og:image e o rodapé dos PDFs". **Controlava três dos quatro:** não havia uma única tag `rel="canonical"` no HTML gerado — nem em 2026-08-07, nem hoje de manhã. O comentário do `publicar/build.mjs` repetia o mesmo erro. As duas frases são minhas. O registro da 0.76 **não foi reescrito**: livro vivo corrige em público, e a correção fica aqui, datada. A partir desta edição a afirmação é verdadeira.
- **Como uma tag inexistente sobreviveu a 428 ocorrências e a um build verde**: nada no processo perguntava se ela existia. O verificador de links conferia destino, o de capítulos conferia estrutura, e ninguém conferia identidade. É o padrão que o cap. 11 descreve — verificação mede o que alguém decidiu medir, e o não-medido é onde o defeito mora. Agora existe `publicar/verifica-canonical.mjs`, no `npm run build`, exigindo que **cada página aponte para si mesma** e que o endereço abandonado não reapareça.
- **Duas cópias vivas do livro, e ninguém tinha notado.** A verificação do estado da migração mostrou o `ghdaru.github.io/harness_engineering` respondendo 200 **e atualizado** — com o apêndice publicado horas antes. A spec 089 decidiu, com dado (47 visitas), abandonar os endereços antigos; o passo do Vercel foi acrescentado na 090 e o do Pages **nunca foi retirado**. Decidir não é executar. Agora o workflow publica num canal só.
- **O default do endereço estava velho** — `SITE_URL` só existia no workflow, então build local e build de CI produziam metadados diferentes, e quem construísse na própria máquina gerava o endereço abandonado. É a armadilha que já pegou este projeto quatro vezes ("variável de ambiente antiga vence default novo"), desta vez pelo avesso: o default é que estava atrasado em relação à realidade.
- **`sitemap.xml` e `robots.txt`** passam a existir, e o sitemap sai da **mesma lista** que o verificador de links já usa — uma enumeração própria divergiria da primeira no primeiro apêndice que alguém acrescentasse, e sitemap que mente sobre o que existe é pior que sitemap nenhum. 109 URLs, os dois idiomas.
- **A regra que o verificador me obrigou a afinar**: ele reprovou `entrar.html` e `sair.html` por falta de canonical. Elas estão certas — são transacionais, de uso único, marcadas `noindex`, e dizer "não me indexe" junto com "esta é a versão canônica" é sinal contraditório. A regra ficou mais estrita, não mais frouxa: página `noindex` é dispensada do canonical **e** precisa estar na lista curta de transacionais, senão o build reprova.
- **Pendente do editor, e sem isso a migração não fecha**: desligar o Pages em *Settings → Pages* (o passo saiu do workflow, mas a última versão publicada continua servida até o desligamento — fechar o repositório também resolve); trocar o token do Vercel `Full Account` por um de escopo do projeto e revogar o provisório; e só **depois** que o endereço antigo sair do ar, limpar `ghdaru.github.io` do `ALLOWED_ORIGINS` no Railway. Na ordem inversa o companion morre em silêncio, que foi a falha da spec 092.
- **Duas vezes no mesmo dia o verificador novo reprovou uma página que estava certa**, e as duas correções deixaram a regra mais estrita, não mais frouxa: `entrar.html`/`sair.html` são `noindex` e por isso dispensadas do canonical (declarar os dois é sinal contraditório) — mas só valem a isenção se estiverem na lista curta de transacionais; e o próprio `HISTORICO` foi reprovado por **citar** o endereço abandonado, que é exatamente o que um livro que corrige em público precisa poder fazer — a regra passou a olhar `href`/`src`/`content`, não prosa. Um verificador que reprova o uso legítimo é desligado, e aí verifica zero.
- **E o CI pegou o que eu não peguei**: rodei o verificador, *depois* escrevi este registro, e não reconstruí. O build local estava medindo um `docs/` anterior ao texto novo. O anti-checklist já dizia para não declarar verificado o que só foi construído; falta a ele a variante de hoje — **não declarar verificado o que foi verificado antes da última mudança**.
- **IA (A3)**: agente **Claude Code (Anthropic)**.

### Edição 0.78 — 2026-08-10 · o livro se mede, tira L2 e conserta (spec 094 · Harness Score)
- **A origem foi um link do editor**, e o que ele trouxe não era uma ferramenta a citar: era **o eixo que faltava**. O benchmark deste livro mede *o harness* (20 sistemas, 12 dimensões); o [Harness Score](https://github.com/paladini/harness-score) (MIT) mede *o repositório do leitor* — quão preparado ele está para ser trabalhado por um agente. O livro ensinava a ler harnesses e a construir um; nunca deu instrumento para o leitor medir **o próprio lado da equação**. Fecha o arco: ler → construir → **medir o seu**.
- **A escolha de projeto é a tese do cap. 11 encarnada**: *"zero LLM calls, zero network access, and the same result every time you run it"*. Um medidor de harness que se recusa a usar modelo para medir harness é o argumento da verificação externa e ancorada virado ferramenta — o número é discutível, mas **reprodutível**, que é o que uma medida precisa ter para sustentar discussão.
- **Medimos o próprio repositório e o resultado foi ruim: L2 · Guided, 59/108.** Contexto 20/20, **sensores 2/20**, **guarda-corpos 0/14** — num repositório com 81 testes, build de quatro passos e uma etapa inteira de evals. O scanner via os arquivos de teste (`SNS-05` passou) e não achava **ponto de entrada na raiz** (`SNS-01` falhou). Compensávamos em **prosa** o que faltava em **configuração**, e é exatamente por isso que contexto deu nota cheia. **Prosa perfeita não substitui guarda-corpo executável**: o agente lê e obedece; um hook, um CI ou o próximo colaborador não leem prosa.
- **O zero em guarda-corpos doeu mais**, num livro que dedica um capítulo a permissões: *nenhuma* regra crítica nossa era imposta por máquina. "Sem segredo em commit" e "sem identificador interno de modelo em artefato" são regras constitucionais — cumpridas por atenção humana, ou seja, cumpridas até a primeira distração. Agora são três hooks, com 37 testes só para eles.
- **A correção teve uma regra antes do trabalho: cada item precisa se justificar sem olhar o placar.** Otimizar para a nota é gamear o verificador — o *reward hacking* que o cap. 11 descreve, cometido pelos autores do capítulo. O que foi recusado está publicado junto: tipagem estrita (4 pts), pré-commit (3), subagentes declarados (5), config de MCP (3), e o badge no README — porque o cap. 01 §6 proíbe agregação numérica espúria e 87/108 não mede a qualidade do livro.
- **Higiene CAIU, de 20/23 para 17/23, e ficou assim.** Antes: *"No dependency manifest detected (nothing to lock)"* — ponto ganho por **ausência**. Depois do manifesto novo, que só tem configuração: *"Manifest present but no lockfile"*. Forjar um lockfile seria mentir para o instrumento. Ficam duas lições que valem para qualquer escada de maturidade: **ponto ganho por ausência não é ponto**, e **o placar não é monótono sob melhoria honesta** — tornar o repositório mais legível fez uma checagem passar a falhar.
- **Resultado**: L2 · 59/108 → **L4 · Self-correcting · 87/108**. Mas o número é resultado, não meta, e o apêndice publica os dois com data, versão e a lista das recusas — sem ela, "subimos de L2 para L4" é propaganda.
- **O checklist pegou uma regressão minha**: os links do apêndice EN caíram no repositório (9 → 11 alvos), pelo mesmo mecanismo registrado na 0.76 — `../benchmark/…` a partir de `livro/en/` não resolve, e o reescritor desiste para o GitHub. Corrigido para o slug traduzido; baseline de **9** restaurado.
- **Verificação**: 118 testes verdes da raiz (81 + 37 novos dos guarda-corpos), build de 4 passos, 30 asserções em navegador nos dois idiomas, corpus regenerado, selos i18n regravados com o hash real.
- **IA (A3)**: agente **Claude Code (Anthropic)**; avaliação da ferramenta, medição, correção do repositório, apêndice e traduções.

### Edição 0.77 — 2026-08-08 · mostrar o progresso é o convite (spec 093 · ADR 0010)
- **O pedido do editor tinha três partes, e a do meio movia uma fronteira**: o convite para guardar progresso estava discreto demais; ele queria **guardar os e-mails** para avisar sobre livros futuros, *"desde que eles aceitem"*; e o leitor deveria **ver o próprio progresso**. O item do meio colidia com o que já estava escrito em três lugares do site — *"sem informativo"* — e com o e-mail que o leitor recebia. A ADR 0009 tinha registrado isso como fronteira e exigido nova ADR para movê-la; a ADR 0010 é essa ADR, e o que ela decide é **como** mover sem quebrar promessa.
...
- **E a correção introduziu uma regressão, pega pelo checklist**: trocar por `en/` fez o reescritor não reconhecer a página e cair no repositório, subindo os alvos de repo de **9 para 10**. O alvo certo era `en/index.html`. É a terceira vez que o item 6 do checklist de verificação pega algo que o build verde deixaria passar.
- **IA (A3)**: agente **Claude Code (Anthropic)**.
...
- **O Pi virou substrato do corpus.** O Prime Agent é o **Pi por baixo** — mesmos quatro pacotes, LICENSE com dupla titularidade (Mario Zechner + Prime Intellect), README creditando o `pi-mono`. É o **quinto** consumidor do Pi no [apêndice da cadeia de suprimentos](apendice-supply-chain.md). A ironia fecha o argumento do cap. 12: o sistema de **menor nota do corpus** (Pi, 26/36 — que recusa metade das dimensões por manifesto) foi a base escolhida por um laboratório de fronteira para o harness mais radical do estudo.
...
- **Feature spec-kit oficial `076-radar-capa-acervo`**, com o editor no papel de aprovador de uma proposta de UX. O [Radar-jornal](radar.html) tinha prazo de validade medido: com uma página só, a fita de abas viraria parede de datas em ~3 semanas e o HTML passaria de **3 MB em um ano**. Agora são duas camadas: **capa** com as 7 edições mais recentes (tamanho constante para sempre) e **acervo mensal** (`radar-AAAA-MM.html`), com o crescimento virando horizontal — nenhuma página passa de ~250 KB, em qualquer horizonte.
...
- **Feature spec-kit oficial `067-livro-en`**: o livro vira **multiidioma** — rodada inglês, em [`/en/`](en/index.html) espelhado com slugs ingleses. **27 páginas traduzidas** (18 capítulos + benchmark + aparato) por 6 agentes em paralelo sob contrato de tradução (glossário fixo, seções canônicas, estrutura 1:1 verificada, citações em inglês verbatim).
