...
## The tool nobody wrote twice

You have four agents on the team: one in the terminal, one in the editor, one in CI, one in Slack. And six internal systems they need to reach: the issue tracker, the staging database, the metrics dashboard, the deploy service, the documentation index, the incident log.

Without a protocol, that is **twenty-four integrations**. Each written in one harness's dialect, maintained by someone, and broken whenever either side changes.

With a protocol, it is **ten**: six servers, one per system, written once; and four clients, one per harness, that already ship. The arithmetic is `M × N` against `M + N`, and it is the only reason this layer exists.

But the number is not the interesting part. Notice what changes hands: the issue tracker's server comes to be maintained by whoever understands issue trackers, not by whoever understands your harness. It is organizational decoupling before it is technical.

And it comes with a bill, which the second half of the chapter covers: **every server is third-party code, and every tool description is third-party text entering your prompt**.
...
## In practice: the adapter that fits in thirty lines

MCP looks big from the outside and is small on the inside. A minimal client, over stdio, makes three calls: it introduces itself, asks what exists, and uses it.
...
That is all. A child process, JSON-RPC line by line, three methods.

Now the part that matters to this book: **wiring it to the ch. 05 `ToolPort` without the loop knowing the difference**.
...
def registrar_servidor(registro, cliente: ClienteMCP, prefixo: str) -> None:
    ...
    for t in cliente.listar():
        registro.adicionar(Tool(
            nome=f"{prefixo}_{t['name']}",          # namespace: two servers may
            descricao=t["description"],             # expose a tool with the same name
            schema=t["inputSchema"],
            executar=lambda args, n=t["name"]: cliente.chamar(n, args),
        ))
```

Seven lines, and the chapter's thesis fits in them: **MCP adds no concept to the loop**. It is transport. A tool arriving over MCP enters the same catalog, with the same contract, and ch. 02 still does not know who implements it.

The prefix is not decoration. Two servers can expose `search`, and without a namespace the second registration silently overwrites the first. That is the kind of defect that only shows up in production, the day someone plugs in the third server.

**And the line missing from that block is the most important in the chapter:**

```python
            executar=lambda args, n=t["name"]: (
                politica.decide(Acao("mcp", prefixo, n, args)) is Veredito.PERMITIR
                and cliente.chamar(n, args)
                or "refused by policy"
            ),
```

The ch. 07 policy applies to MCP tools **on the client side**. There is no point trusting that the server behaves: whoever produces the effect on your machine is you, and the decision is yours.

One detail most tutorials omit. The `t["description"]` in the block above is **text written by a third party that enters your system prompt**. It is not documentation: it is untrusted content, in the exact sense of ch. 07. A compromised server does not need to export any malicious tool, it only needs to describe an innocent one with instructions addressed to the model.
...
The protocol turned a page on 2026-07-28: a **stateless** core (no handshake, no `Mcp-Session-Id`), MRTR in place of server-initiated sampling/elicitation, formal extensions, caching as a contract (`ttlMs`) and the first deprecation policy (12 months), under which Sampling, Roots, Logging and the HTTP+SSE transport fall. What the cohort *runs today* is still the protocol of sections 1–5 (the window exists for that); what you *write today* should already target 2026-07-28. **What to steal:**

- **Treat the tool description as untrusted input.** The literature measures about 73% success for *tool poisoning*.
- **In new code, prefer stateless Streamable HTTP.** The SSE fallback is already a deprecated transport.
- **If you expose an MCP server, filter the subprocess environment** and never let the server see a credential.
- **Pin the protocol revision you speak**, and negotiate it explicitly.
- **Apply the ch. 07 policy on the client side**, because whoever produces the effect is you.
...
1. Why does MCP reduce integration cost from M×N to M+N, and what does that have to do with "open standard"?
2. You are about to connect a third-party MCP server exposing a `search_tickets` tool. Give two reasons to distrust it and two concrete defenses.
3. A harness that is both MCP **client AND server** gains what that a client-only one does not, and which protocol primitives does that activate?
...
**1.** Because without a standard each harness needs one integration per system, and each system needs one per harness: with M harnesses and N systems, someone writes and maintains M×N adapters. With a common protocol, each system writes **one** server and each harness writes **one** client, and the total drops to M+N. The "open" part is what makes the arithmetic hold: if the protocol belonged to one vendor, the other M−1 harnesses would not implement it, and you would be back to dialect integrations with a prettier name. The real gain, incidentally, is organizational before it is arithmetic — the server comes to be maintained by whoever understands the system, not the harness.

**2.** Two reasons to distrust it. First, **the tool description enters your system prompt**: it is third-party text aimed at the model, and a compromised server can instruct the agent without exposing any malicious tool, which the literature calls *tool poisoning*. Second, **the return value also enters the context**: it is indirect injection through the front door, exactly the ch. 07 vector. And the server registry is itself a supply-chain vector, as the appendix records.

Two concrete defenses. **The ch. 07 policy applied on the client side**, before executing any MCP call, because whoever produces the effect on your machine is you. And **pinning the server version**, reviewing what changed between versions, instead of accepting automatic updates of third-party code running in your environment. The defense that does **not** work is trusting the description: it is precisely what the attacker controls.

**3.** It stops being only a consumer and becomes a **consumable part** for other agents: your harness becomes pluggable into any MCP client, including other harnesses, which is the same composition move ch. 17 describes at the ACP boundary. On the protocol side, being a server activates the primitives where the **server asks the client** — the server requesting an inference from the client's model, and the server asking the user for information through the client. Both invert the direction of the conversation, which is why a client-only harness never needs to implement them. The period caveat: the 2026-07-28 spec changed exactly this area, and what holds today is the version your SDK implements, not the one the chapter describes.
