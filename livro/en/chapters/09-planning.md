<!-- i18n fonte:livro/capitulos/09-planejamento.md edicao:0.88 hash:1f42780d -->
# 09. Planning
...
- **Thinking between actions**: [The "think" tool](https://www.anthropic.com/engineering/claude-think-tool) adds a reasoning step *in the middle* of tool use (after the result arrives). [extended thinking](https://docs.claude.com/en/docs/build-with-claude/extended-thinking) exposes reasoning blocks with `budget_tokens` and, in the 4-series models, **interleaved thinking** (think → call tool → think about the result → call again). Decision: allocate explicit budget to planning steps and let reasoning interleave with tools, planning is continuous, not a one-shot prefix. *(anthropic.com returns 403 through the proxy; confirmed via independent mirrors.)*
...
Step 8 (`harness-zero/etapas/08-plan/`) adds plan mode to harness-zero by **reusing** the `PermissionPolicy` from step 6: entering plan mode sets a mode the policy translates into "every write tool is denied". The agent only reads and proposes; exiting asks for approval and restores the mode. It is the concrete demonstration of the chapter's thesis, plan mode is a configuration of the permission domain that already exists, not a new subsystem. Completeness exercise: `propor_plano` already persists the artifact (`PLAN.md`); you add the requirement that exiting plan mode only happens with an approved `PLAN.md`, the gate between planning and executing.
