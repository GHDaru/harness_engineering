<!-- i18n fonte:livro/capitulos/12-extensibilidade.md edicao:0.88 hash:0061cbfb -->
# 12. Extensibility
...
- **Settings as an enforcement surface**: [Claude Code's config](https://code.claude.com/docs/en/settings) is a precedence stack (Managed &gt. CLI &gt; local &gt; project &gt; user); most keys override, but **permission rules merge**, and **managed settings cannot be overridden** (a security team denies tools/marketplaces for the whole company). Decision: config is enforcement, not preference (connects to ch. 07).
