...
## Caps. 13, 15, 17 — a lacuna registrada

Interfaces, harnesses embutidos e protocolos têm literatura acadêmica **rarefeita** (buscas de 2026-07 não retornaram surveys dedicados). Registro editorial: o livro cobre essas dimensões com specs, evidência do benchmark e literatura industrial — e assinala a lacuna como oportunidade de pesquisa (possível seção "problemas em aberto" no cap. 14). Nota: o cap. 12 (extensibilidade), antes nesta lista, foi ancorado na SE clássica de arquiteturas extensíveis (ver a seção Cap. 12 acima) — a lacuna *agent-specific* persiste, mas as fundações duráveis existem.
