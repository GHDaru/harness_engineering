...
> **Adendo (2026-07-31): a terceira via — compactação aprendida no treino.** O preprint [CompactionRL](https://arxiv.org/abs/2607.05378) (jul/2026) propõe o passo seguinte da migração: treinar o modelo por RL **com a compactação dentro do loop** — o rollout é compactado quando o orçamento esgota e a trajetória continua; sumarização vira "a learned part of the model rather than an inference-time heuristic", com recompensa de nível de **tarefa** (não de fidelidade do resumo). Reporta +7,0 pontos de Pass@1 no SWE-bench Verified (GLM-4.5-Air). Duas consequências para este capítulo, se o resultado se sustentar: (1) o harness continua dono do *quando*, mas o *como sumarizar* migra para os pesos — e o descasamento harness↔modelo vira risco novo (modelo treinado num protocolo de compactação, operado noutro, roda fora da distribuição); (2) a limitação declarada é reveladora: os ganhos **não transferem** quando a compactação é desligada — compactação treinada cria *acoplamento*, o argumento mais forte até agora para a compactação nativa de provedor como caminho in-distribution. *(Avaliação pelo abstract e fontes secundárias — arXiv inacessível deste ambiente de edição; leitura integral na janela 2026-10.)*
