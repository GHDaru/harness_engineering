...
1. **Explicar** por que o MCP virou a língua franca da integração de agentes — o argumento do padrão aberto contra o custo M×N de integrações ponto a ponto;
2. **Comparar** os transportes do protocolo (stdio, Streamable HTTP, SSE depreciado) e decidir qual usar para servidor local × remoto;
...
- **Injeção indireta de prompt** — [Greshake et al., arXiv 2302.12173](https://arxiv.org/abs/2302.12173) (AISec '23), o paper que definiu a ameaça: aplicações integradas a LLM borram a fronteira entre *dados* e *instruções*, então qualquer conteúdo recuperado é um canal de instrução em potencial. Traduzido para o cliente MCP: o campo de descrição de uma tool e o texto que o servidor devolve são **dados**, nunca instruções confiáveis.
...
- **[Arquitetura do MCP](https://modelcontextprotocol.io/docs/learn/architecture)** (spec oficial): define o que a harness precisa mapear — no servidor, *tools/resources/prompts*; no cliente, *roots/sampling/elicitation*. A decisão de design: separar o que o servidor *oferece* do que o cliente *concede* — `sampling` e `roots` existem para o servidor pedir uma inferência ou um escopo de arquivos **sem nunca ter acesso direto** ao modelo ou ao filesystem, mantendo o host como único ponto de confiança.
...
- **Mediar credenciais**: o IronClaw adapta tools MCP a *capabilities* **sem conceder autoridade ambiente** — FS, segredos e rede continuam mediados, e **o servidor nunca vê o secret** (a credencial é injetada pela borda). O OpenHands **reda e restaura segredos** em round-trips de configuração.
...
O que está mais moderno: o transporte convergiu (stdio + Streamable HTTP); o harness fechou o ciclo cliente↔**servidor**, ganhando *sampling* e *elicitation*; a autenticação empresarial virou a fronteira de features; e a contenção do servidor de terceiros (env filtrado, credencial mediada, eval de injeção) deixou de ser exótica. **O que roubar:** trate a descrição de tool como input não-confiável (a literatura mede ~73% de sucesso de *tool poisoning*); fale Streamable HTTP com fallback SSE; se for expor um servidor MCP, filtre o ambiente do subprocesso e nunca deixe o servidor ver segredos; e, se seu público é enterprise, OAuth 2.1 com impersonation é o piso.

## Mão na massa — harness-zero, etapa 7

A etapa 7 (`harness-zero/etapas/07-mcp/`) dá ao harness-zero um **adapter MCP client (stdio)** atrás de uma porta. Fiel à arquitetura hexagonal *por refatoração*: a `ToolPort` da etapa 2 já define o que é uma tool; agora um adapter descobre tools de um servidor MCP externo (via `stdio`, lançando o processo) e as apresenta ao loop como tools nativas — o modelo não distingue. Você conecta um servidor MCP real (por exemplo, um servidor de filesystem oficial), lista suas tools, e as chama pelo mesmo caminho das tools locais. Exercício de completude: o cliente trata o *happy path*; você adiciona a **degradação graciosa** (um servidor que cai não derruba a sessão) e um **filtro de env** no subprocesso stdio — a defesa mínima do estado da arte.
