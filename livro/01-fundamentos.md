...
> **A definição está convergindo por fora, e de cinco lados que não se coordenaram.** (1) A Microsoft, ao anunciar seu Agent Framework, define harness como *"the **scaffolding** that turns a language model into an agent"*; (2) o survey de Meng et al. formaliza `H = (E, T, C, S, L, V)` — loop, ferramentas, contexto, estado, ciclo de vida, avaliação; (3) o survey do RUCAIBox organiza o campo em quatro eixos com a mesma espinha; (4) o diretório `best-of-Agent-Harnesses` sintetiza *"the model thinks; the harness decides what that thinking is allowed to touch"* — permissão dentro da definição, não como capítulo à parte; e (5) a mais operacional de todas, porque define pelo que se pode **verificar no sistema de arquivos**: o [Harness Score](https://github.com/paladini/harness-score) mede seis dimensões — contexto, skills, guarda-corpos, sensores, CI, higiene — que caem quase limpo sobre os capítulos deste livro (ver o [Apêndice — Meça o seu harness](apendice-harness-score.md)). Cinco grupos independentes chegando aos mesmos elementos é o melhor argumento disponível de que a definição descreve algo real, e não um recorte editorial nosso. As quatro peças acima são a forma que este livro dá a esse consenso.
