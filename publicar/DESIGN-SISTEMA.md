...
|---|---|---|---|---|---|
| C01 | **CabeçalhoDeCapítulo** | identidade do capítulo: número, título, teaser (Backward Design: o "para onde vamos") + datação (Princípio IV) + carga estimada (Sweller: expectativa de esforço) | item do `sumario.json` (capítulo numerado) + blockquote de data | `.cap-hero` | 🔶 variante em aprovação (A/B/C) |
...
| C08 | **LeituraExecutiva** | síntese acionável do estado da arte | `### Leitura executiva` | `.leitura-exec` | 🔜 (hoje sem tratamento) |
...
| N02 | **PaginaçãoEmCartões** | anterior/próximo na linguagem do cartão | `.pagcards` | 🔜 (spec 043; hoje links planos) |
